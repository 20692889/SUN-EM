function [Zx, Zy, Zz] = buildMoMblock (Const,Solver_setup, zMatrices, freq, ObservRWGs, SourceRWGs)
   ...
   num_dofs = Solver_setup.num_metallic_edges;       % Replacing global NUM_DOFS
    fieldDOF = numel(ObservRWGs);          % Number of RWG edges in the far field
    sourceDOF = numel(SourceRWGs);
    startO = 1;
    startS = 1;
   ...
   if(sourceDOF == 1)
    ...
    Zx.nBasis  = sourceDOF; % number of cols (basis/source  functions)
    
    Zy.numFreq = number_of_frequencies;
    Zy.mBasis  = fieldDOF; % number of rows (testing/field functions)
    Zy.nBasis  = sourceDOF; % number of cols (basis/source  functions)
    
    Zz.numFreq = number_of_frequencies;
    Zz.mBasis  = fieldDOF; % number of rows (testing/field functions)
    Zz.nBasis  = sourceDOF; % number of cols (basis/source  functions)
    
    % Allocate some space for our impedance matrix
    Zx.values = complex(zeros(fieldDOF,sourceDOF,number_of_frequencies)); % Generalized impedance matrix.
    Zy.values = complex(zeros(fieldDOF,sourceDOF,number_of_frequencies)); % Generalized impedance matrix.
    Zz.values = complex(zeros(fieldDOF,sourceDOF,number_of_frequencies)); % Generalized impedance matrix.
    ...
    for freq_index = 1:number_of_frequencies
        ...
        for mm = startO:fieldDOF %step through observation points

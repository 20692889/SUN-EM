function [Z] = buildMoMblock (Const,num,Solver_setup, zMatrices, freq, ObservRWGs, SourceRWGs)
   ...
   if(sourceDOF == 1)
    ...
    Z.nBasis  = sourceDOF; % number of cols (basis/source  functions)
   ...
   % Zy.mBasis  = fieldDOF; % number of rows (testing/field functions)
    ...
    Z.values = complex(zeros(fieldDOF,sourceDOF,number_of_frequencies)); % Generalized impedance matrix.
